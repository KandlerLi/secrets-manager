...
`rotation-schedule.json` for browsing on GitHub -- a Markdown table,
sorted soonest-to-expire first, since GitHub renders `.json` as
syntax-highlighted code (never Markdown) and plain GFM tables have no
built-in column sort to fall back on. Generated by
...
regenerate just the Markdown after hand-editing the JSON (e.g. adding
a new secret) without rotating anything.
